...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a) <= 1.0 and sigmoid(a) >= 0.0  # Check bounds
    assert_close(1 - sigmoid(a), sigmoid(-a))  # Check 1-sig(x) = sig(-x)
    assert_close(sigmoid(0), 0.5)  # Check sig(0) = 0.5
    assert sigmoid(a) * (1 - sigmoid(a)) >= 0.0  # Check for strict increasing
...
def test_other(a: float) -> None:
    ...
    assert_close(relu(a), 0 if a < 0 else a)
    assert_close(add(a, neg(a)), 0)
